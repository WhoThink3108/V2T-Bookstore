...
<style>
    ...
    /* FIX LINTER DÒNG 90: Đưa trạng thái xác minh mua hàng vào CSS Class tách biệt */
...
<div style="width: 100%;">
    ...
    <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 28px;">
        <div class="rev-card">
            ...
            <h2 style="font-size: 1.6rem; font-weight: 700; margin: 8px 0 0 0; color: #111827;">{{ number_format($totalCount) }}</h2>
        </div>
        <div class="rev-card">
            <span style="font-size: 0.75rem; color: #6b7280; font-weight: 700; text-transform: uppercase;">Chờ duyệt</span>
            <div style="display: flex; justify-content: space-between; align-items: baseline; margin-top: 8px;">
                <h2 style="font-size: 1.6rem; font-weight: 700; margin: 0; color: #b91c1c;">{{ $pendingCount }}</h2>
                @if($pendingCount > 0)<span style="font-size: 0.7rem; font-weight: 700; color: #b91c1c; background: #fee2e2; padding: 2px 6px; border-radius: 4px;">Cần xử lý</span>@endif
            </div>
        </div>
        <div class="rev-card">
            <span style="font-size: 0.75rem; color: #6b7280; font-weight: 700; text-transform: uppercase;">Điểm trung bình</span>
            <h2 style="font-size: 1.6rem; font-weight: 700; margin: 8px 0 0 0; color: #92400e;">{{ $avgRating }} <span style="color: #fbbf24; font-size: 1.2rem;">★</span></h2>
        </div>
        <div class="rev-card">
            <span style="font-size: 0.75rem; color: #6b7280; font-weight: 700; text-transform: uppercase;">Bị báo cáo vi phạm</span>
            <h2 style="font-size: 1.6rem; font-weight: 700; margin: 8px 0 0 0; color: #374151;">{{ $violatedCount }}</h2>
        </div>
    </div>

    <div style="display: flex; gap: 10px; margin-bottom: 24px; background: white; padding: 12px; border-radius: 12px; border: 1px solid #e5e7eb;">
    ...
    <div style="width: 100%;">
        ...
        <div class="review-item-box {{ $review->status === 'violated' ? 'review-item-violated' : '' }}">
            
            <span class="status-lbl {{ $review->status === 'pending' ? 'lbl-pending' : ($review->status === 'approved' ? 'lbl-approved' : 'lbl-violated') }}">
                {{ $review->status === 'pending' ? 'Chờ duyệt' : ($review->status === 'approved' ? 'Đã duyệt' : 'Vi phạm') }}
            </span>
